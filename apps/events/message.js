...
Bot.on?.("message", async (e) => {
    ...
    // 判断是否为频道消息（排除频道消息）
    if (e.message_type === "guild") return false;
    // 判断是否为群聊或私聊消息
    if (e.message_type !== "group" && e.message_type !== "private") return false;
    // 判断是否为主人消息
    ...
    // 发送消息
    if (msg) await common.sendMasterMsg(msg, (e.bot ?? Bot).uin);
