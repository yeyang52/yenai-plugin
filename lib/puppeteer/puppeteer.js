...
export default new (class newPuppeteer {
  ...
  async launch () {
    ...
    if (!puppeteer) {
      logger.error(`[椰奶][Puppeteer]缺少依赖项 请到 ${logger.yellow('yenai-plugin/lib/puppeteer')} 执行 ${logger.red('pnpm i')} 或直接执行 ${logger.red('pnpm add puppeteer-extr puppeteer-extra-plugin-stealth -w')}`)
      return false
    }
